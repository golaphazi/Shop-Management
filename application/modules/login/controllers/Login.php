...
class Login extends CI_Controller {
    ...
    public function __construct() {
		...
		$this->load->model(array('Login_model' => 'login'));
	}
		...
		if(isset($_POST['login_data'])){
			...
			if(strlen($login_id) > 0){
				if(strlen($password) > 0){
					$this->login->login_user($login_id,$password);					
				}else{
					$this->session->set_flashdata('messagePr', 'Enter your user password..');	
				}
			}else{
				$this->session->set_flashdata('messagePr', 'Enter your user id..');	
			}
		}
