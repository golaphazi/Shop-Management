...
Class Purchase_model Extends CI_model {
	...
	private function items_data_value($id=0, $offset=0, $show = 'pagi'){
		if($id > 0){
			$search = $this->db->query("SELECT * FROM product_items AS item INNER JOIN setting_item_type AS type ON item.ITEM_TYPE = type.ITEM_TYPE_ID INNER JOIN setting_item_unit AS unit ON item.ITEM_UNIT = unit.ITEM_UNIT_ID WHERE item.ITEM_ID = '".$id."' AND item.ITEM_STATUS = 'Active' AND item.COMPANY_ID = '".$this->companyID."'");
		}else{
			if($show == 'all'){
				$search = $this->db->query("SELECT * FROM product_items AS item INNER JOIN setting_item_type AS type ON item.ITEM_TYPE = type.ITEM_TYPE_ID INNER JOIN setting_item_unit AS unit ON item.ITEM_UNIT = unit.ITEM_UNIT_ID WHERE item.ITEM_STATUS = 'Active' AND item.COMPANY_ID = '".$this->companyID."'  ORDER BY item.ITEM_NAME ASC");
			}else{
				$search = $this->db->query("SELECT * FROM product_items AS item INNER JOIN setting_item_type AS type ON item.ITEM_TYPE = type.ITEM_TYPE_ID INNER JOIN setting_item_unit AS unit ON item.ITEM_UNIT = unit.ITEM_UNIT_ID WHERE item.ITEM_STATUS = 'Active' AND item.COMPANY_ID = '".$this->companyID."'  ORDER BY item.ITEM_ID DESC LIMIT $offset,10");
			}
	...
	private function items_model_data_value($id=0, $offset=0, $show = 'pagi'){
		if($id > 0){
			$search = $this->db->query("SELECT * FROM product_items_model AS model INNER JOIN setting_item_type AS type ON model.ITEM_TYPE = type.ITEM_TYPE_ID INNER JOIN setting_item_unit AS unit ON model.ITEM_UNIT = unit.ITEM_UNIT_ID WHERE model.MODEL_ID = '".$id."' AND model.MODEL_STATUS = 'Active' AND model.COMPANY_ID = '".$this->companyID."'");
		}else{
			if($show == 'all'){
				$search = $this->db->query("SELECT * FROM product_items_model AS model INNER JOIN setting_item_type AS type ON model.ITEM_TYPE = type.ITEM_TYPE_ID INNER JOIN setting_item_unit AS unit ON model.ITEM_UNIT = unit.ITEM_UNIT_ID WHERE model.MODEL_STATUS = 'Active' AND model.COMPANY_ID = '".$this->companyID."'  ORDER BY model.MODEL_CODE ASC");
			}else{
				$search = $this->db->query("SELECT * FROM product_items_model AS model INNER JOIN setting_item_type AS type ON model.ITEM_TYPE = type.ITEM_TYPE_ID INNER JOIN setting_item_unit AS unit ON model.ITEM_UNIT = unit.ITEM_UNIT_ID WHERE model.MODEL_STATUS = 'Active' AND model.COMPANY_ID = '".$this->companyID."'  ORDER BY model.MODEL_ID DESC LIMIT $offset,10");
			}
